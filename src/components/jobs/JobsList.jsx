      ...
      <Droppable droppableId="jobs-list">
        {(provided, snapshot) => (
          <div
            ...
            className={`space-y-4 ${snapshot.isDraggingOver ? 'bg-gray-50 rounded-lg p-4' : ''} flex min-w-[75vw] lg:flex-wrap flex-col lg:flex-row gap-4 min-h-[10vh] items-center`}
          >
            {jobs.map((job, index) => (
              <Draggable key={job.id} draggableId={job.id.toString()} index={index}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={snapshot.isDragging ? 'rounded-lg shadow-lg mt-4' : 'mt-4'}
                  >
                    <JobCard 
                      job={job} 
